...
- **Patient browser** -- Vite + React frontend (Wouter routing) + embedded Stitch widget.
...
All backend code lives under `artifacts/crosscures-api/`.
...
All frontend code lives under `artifacts/crosscures/`. The frontend uses
Vite + React with Wouter for routing (not Next.js).
...
- Workspace > Webhooks: create a webhook subscribed to these four events:
  ...
  - `ehi_export_failed`
  - `webhook.test`
- Workspace > Redirect URL: set to `http://localhost:5173/callback`.
...
Add to `artifacts/crosscures-api/.env`:
...
Add to `artifacts/crosscures/.env.local`:

```
VITE_FASTEN_PUBLIC_ID=public_test_...
PORT=5173
BASE_PATH=/
```

The `VITE_` prefix is required for Vite to expose env vars to
client-side code via `import.meta.env`. Same value as backend's
`FASTEN_PUBLIC_ID`. `PORT` and `BASE_PATH` are required by
`vite.config.ts`. Restart the dev server after adding (env changes
need restart, not just hot reload).

### 5. Run it

```bash
# Terminal 1 — ngrok tunnel for webhooks
ngrok http 8000

# Terminal 2 — backend (activate venv first)
cd artifacts/crosscures-api
source ../../.venv/bin/activate
uvicorn main:app --reload --port 8000

# Terminal 3 — frontend (requires Node 20 via Volta)
cd <project-root>
PORT=5173 BASE_PATH=/ volta run --node 20.20.2 pnpm --filter @workspace/crosscures dev
```
...
**Note:** The SQLite database (`crosscures.db`) must exist in
`artifacts/crosscures-api/`. If login fails with "Incorrect email or
password", run `python seed_demo.py` from that directory (with venv
activated) to create demo accounts.
...
1. Open http://localhost:5173, log in as a patient (demo seed creates
...
The frontend reads `VITE_FASTEN_PUBLIC_ID` from
`artifacts/crosscures/.env.local` via `import.meta.env`. Pydantic
Settings on the backend rejects unknown env vars by default and will
crash on startup if the var is in the backend `.env` instead. We
mitigated by setting `extra = "ignore"` in `Settings.Config`, so
backend now tolerates stray vars -- but the right place for frontend
env vars is still `artifacts/crosscures/.env.local`.
...
7. **Move Stitch CDN load out of the FastenConnect component.**
   ...
   document head on every component mount. Better to load via a
   top-level effect or include directly in `index.html`.
