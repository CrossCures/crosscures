...
export default function FastenConnect({ onComplete }: { onComplete?: () => void }) {
  ...
  const publicId = import.meta.env.VITE_FASTEN_PUBLIC_ID || "";
  ...
  useEffect(() => {
    ...
    const handler = (event: Event) => {
      ...
      if (normalized === "connection_pending") {
        ...
        setStatus("failed");
        setErrorMsg("Widget misconfigured — check VITE_FASTEN_PUBLIC_ID");
      }
    };
  ...
  if (!publicId) {
    return (
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700">
        ...
        VITE_FASTEN_PUBLIC_ID is not set in .env.local
      </div>
    );
  }
  if (!user) return null;
