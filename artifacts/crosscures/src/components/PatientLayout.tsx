        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => (
            <Link
              ...
              className={cn(
                "sidebar-item",
                location === item.href || location.startsWith(item.href + "/") ? "active" : ""
              )}
            >
              <item.icon className="w-5 h-5 flex-shrink-0" />
              <span>{item.label}</span>
              {(location === item.href || location.startsWith(item.href + "/")) && (
                <ChevronRight className="w-4 h-4 ml-auto text-crosscure-500" style={{ color: 'var(--color-crosscure-500)' }} />
              )}
            </Link>
          ))}
        </nav>
            ...
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
              {navItems.map((item) => (
                <Link
                  ...
                  className={cn("sidebar-item", location === item.href ? "active" : "")}
                >
