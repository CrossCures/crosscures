  ...
  const Sidebar = () => (
    <aside className="flex flex-col w-64 bg-slate-900 h-full">
      <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-700">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-sm" style={{ backgroundColor: 'var(--color-crosscure-500)' }}>
      ...
      <nav className="flex-1 p-4 space-y-1">
        {navItems.map((item) => (
          <Link
            ...
            style={location === item.href || location.startsWith(item.href + "/") ? { backgroundColor: 'var(--color-crosscure-600)' } : {}}
          >
      ...
      <div className="p-4 border-t border-slate-700">
        <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-800 cursor-pointer">
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-semibold text-sm flex-shrink-0" style={{ backgroundColor: 'var(--color-crosscure-700)' }}>
    ...
    <div className="flex h-screen bg-slate-100 overflow-hidden">
      ...
      <div className="lg:hidden fixed inset-x-0 top-0 z-50 bg-slate-900 flex items-center justify-between px-4 h-14">
        <div className="flex items-center gap-2">
          <Stethoscope className="w-6 h-6" style={{ color: 'var(--color-crosscure-400)' }} />
