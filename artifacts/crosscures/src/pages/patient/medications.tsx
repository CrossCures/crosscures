...
import { Pill, Plus, Loader2, CheckCircle2 } from "lucide-react";
  ...
  return (
    <PatientLayout>
      <div className="max-w-3xl mx-auto px-4 py-8 lg:px-8">
        ...
        {showAdd && (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-lg p-6 mb-6 animate-fade-in" style={{ borderColor: 'var(--color-crosscure-200)' }}>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {prescriptions.map((rx) => (
              <div key={rx.id} className="bg-white rounded-2xl p-5 border border-slate-100 hover:border-slate-200 transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      {rx.prescribing_physician && <p className="text-slate-400 text-xs mt-1">Prescribed by {rx.prescribing_physician}</p>}
                      <p className="text-slate-400 text-xs mt-0.5">Started {formatDate(rx.start_date)}</p>
                    </div>
                  </div>
                  <span className={cn("badge text-xs flex-shrink-0", statusColor[rx.status] || "bg-slate-100 text-slate-600")}>{rx.status}</span>
                </div>
              </div>
            ))}
          </div>
