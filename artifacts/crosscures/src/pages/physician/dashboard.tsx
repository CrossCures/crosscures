...
import { FileText, Bell, Users, AlertTriangle, CheckCircle2, ChevronRight, Loader2 } from "lucide-react";
...
export default function PhysicianDashboard() {
  ...
  useEffect(() => {
    ...
    if (user.role !== "physician") { navigate("/patient/home"); return; }
    physicianApi.getDashboard().then((res) => setDashboard(res.data)).finally(() => setLoading(false));
  }, [user, navigate]);
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Linked Patients", value: dashboard?.linked_patient_count ?? 0, icon: Users, color: "bg-slate-100", iconColor: "text-slate-600", crosscure: true },
                { label: "Unread Briefs", value: dashboard?.unread_briefs?.length ?? 0, icon: FileText, color: "bg-teal-100", iconColor: "text-teal-600" },
                { label: "Pending Alerts", value: dashboard?.unacknowledged_alerts?.length ?? 0, icon: Bell, color: dashboard?.unacknowledged_alerts?.length > 0 ? "bg-red-100" : "bg-slate-100", iconColor: dashboard?.unacknowledged_alerts?.length > 0 ? "text-red-600" : "text-slate-500" },
              ].map((stat) => (
                <div key={stat.label} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                  <div className={cn("w-11 h-11 rounded-xl flex items-center justify-center mb-3", stat.crosscure ? "" : stat.color)}
                    style={stat.crosscure ? { backgroundColor: 'var(--color-crosscure-100)' } : {}}>
                    <stat.icon className={cn("w-5 h-5", stat.crosscure ? "" : stat.iconColor)}
                      style={stat.crosscure ? { color: 'var(--color-crosscure-600)' } : {}} />
                  </div>
            ...
            {dashboard?.unacknowledged_alerts?.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link href="/physician/alerts" className="text-sm hover:underline flex items-center gap-1" style={{ color: 'var(--color-crosscure-600)' }}>
                  ...
                  {dashboard.unacknowledged_alerts.slice(0, 5).map((alert: any) => (
                    <Link key={alert.alert_id} href={`/physician/alerts/${alert.alert_id}`}>
                      <div className="bg-white rounded-2xl p-4 border border-red-100 hover:border-red-300 hover:shadow-sm transition-all flex items-center gap-4 group">
                        <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0", alert.severity === "severe" ? "bg-red-100" : alert.severity === "moderate" ? "bg-orange-100" : "bg-yellow-100")}>
                          <Bell className={cn("w-5 h-5", alert.severity === "severe" ? "text-red-600" : alert.severity === "moderate" ? "text-orange-600" : "text-yellow-600")} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {alert.requires_acknowledgment && <span className="badge bg-red-100 text-red-700 text-xs">Requires ACK</span>}
                          </div>
            ...
            {dashboard?.unread_briefs?.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link href="/physician/briefs" className="text-sm hover:underline flex items-center gap-1" style={{ color: 'var(--color-crosscure-600)' }}>
                    View all <ChevronRight className="w-4 h-4" />
                  </Link>
                </div>
                <div className="space-y-3">
            ...
            {!dashboard?.unacknowledged_alerts?.length && !dashboard?.unread_briefs?.length && (
              <div className="text-center py-16 bg-white rounded-3xl border border-slate-100">
                ...
                {dashboard?.linked_patient_count === 0 && (
                  <p className="text-sm text-slate-400 mt-2">Ask your patients to link you in their app settings to get started.</p>
                )}
              </div>
            )}
          </>
