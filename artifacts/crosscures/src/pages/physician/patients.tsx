...
import { Users, FileText, ChevronRight, Loader2, AlertTriangle } from "lucide-react";
...
export default function PatientsPage() {
  ...
  useEffect(() => {
    ...
    physicianApi.getPatients().then((res) => setPatients(res.data.patients || [])).finally(() => setLoading(false));
  }, [user, navigate]);
          ...
          <div className="space-y-3">
            {patients.map((patient) => (
              <Link key={patient.id} href={`/physician/patients/${patient.id}`}>
                <div className="bg-white rounded-2xl p-5 border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all flex items-center gap-4 group">
                  <div className="w-12 h-12 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0" style={{ backgroundColor: 'var(--color-crosscure-100)', color: 'var(--color-crosscure-700)' }}>
