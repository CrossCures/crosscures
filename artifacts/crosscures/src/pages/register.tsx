...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-crosscure-50 to-teal-50 p-4" style={{ background: 'linear-gradient(135deg, var(--color-crosscure-50) 0%, #f0fdfa 100%)' }}>
      <div className="w-full max-w-lg bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
        <div className="gradient-bg p-8 text-white">
          ...
          <div className="flex gap-2 mt-4">
            {[1, 2, 3].map((s) => (
              <div key={s} className={cn("h-1.5 rounded-full flex-1 transition-all", s <= step ? "bg-white" : "bg-white/30")} />
            ))}
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
                ...
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      form.role === opt.role ? "border-slate-200 bg-slate-50" : "border-slate-200 hover:border-slate-300 bg-white"
                    )}
                    style={form.role === opt.role ? { borderColor: 'var(--color-crosscure-500)', backgroundColor: 'var(--color-crosscure-50)' } : {}}
                  >
          ...
          {step === 2 && (
            <div className="space-y-5 animate-fade-in">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Please review and configure your consent settings.</p>
              </div>
              <div className="space-y-3">
                {CONSENT_ACTIONS.map((item) => (
                  <label key={item.key} className={cn(
                    "flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all",
                    consents[item.key] ? "border-slate-200 bg-slate-50" : "border-slate-200 bg-white hover:bg-slate-50"
                  )}
                    style={consents[item.key] ? { borderColor: 'var(--color-crosscure-200)', backgroundColor: 'var(--color-crosscure-50)' } : {}}
                  >
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.required && <p className="text-xs mt-0.5" style={{ color: 'var(--color-crosscure-600)' }}>Required</p>}
                    </div>
                  </label>
                ))}
              </div>
              <p className="text-xs text-slate-400">All data is encrypted at rest (AES-256) and in transit (TLS 1.3).</p>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-5 animate-fade-in">
              ...
              <p className="text-sm text-slate-500">By creating an account you agree to our Terms of Service and Privacy Policy.</p>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-2">
            ...
            {step < 3 ? (
              ...
              <button className="btn-primary flex-1" onClick={handleRegister} disabled={loading}>
                {loading ? (
          ...
          <p className="text-center text-sm text-slate-500">
            ...
            <Link href="/login" className="font-semibold hover:underline" style={{ color: 'var(--color-crosscure-600)' }}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
